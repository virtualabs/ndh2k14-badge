...
void xmitCodeElement(uint16_t ontime, uint16_t offtime, uint8_t PWM_code )
{
  ...
  PORTD &= ~(_BV(IRLED));           // turn off IR LED
...
int main(void) {
  ...
  DDRB = _BV(LED);   // set the visible and IR LED pins to outputs
  PORTB = _BV(LED);
...
void tvbgone_sleep( void )
{
  ...
  TCCR2A = 0;           // turn off frequency generator (should be off already)
  TCCR2B = 0;           // turn off frequency generator (should be off already)
